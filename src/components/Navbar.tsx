...
const Navbar = (props: searchProp) => {
  ...
  return (
    <>
      ...
      <div className=" flex justify-between text-sm font-bold text-gray-700 items-center  p-4 shadow-lg">
        <Link to='/main'><img src={logo} className='w-20 h-12 ml-5' alt=""/></Link>
        ...
        <div className='border  border-gray-100 flex items-center bg-gray-100 w-96 h-10 ml-14'>
          <img src={lens} className='h-3 w-3 ml-4' alt=""/>
          <input onChange={(e) => props.setSearch(e.target.value)} className="bg-gray-100  text-gray-900 font-normal outline-none text-sm rounded-sm  block w-full p-2.5 ml-3" placeholder="search for products" required />
        </div>

        {auth.currentUser?.phoneNumber ? <div onClick={logout} className='ml-6 text-xs'>
          <img src={out} className='w-4 h-4 ml-3' alt="" />
          <h1 className='cursor-pointer'>Logout</h1>
        </div>
        
        : <Link to='/login'>
            <div className='ml-6 text-xs'>
              <img src={account} className='w-5 h-5 ml-2' alt="" />
              <h1 className='cursor-pointer'>Login</h1>
            </div>
          </Link>}
        <div onClick={wishlist} className='ml-6 text-xs '>
          <img src={heart} className='w-5 h-5 ml-3' alt=""/>
